...
var search_for_location = [
"Haben Sie Ihre Gemeinde nicht gefunden? Wenn Sie dennoch Eintragungen machen möchten, klicken Sie hier, um ihre Gemeinde anschließend durch Klicken auf die Karte zu lokalisieren.",
"Est-ce que vous n´avez pas trouvé votre commune? En cas que vous vouliez insérer un mot quand même, cliquez ici pour localiser ensuite votre commune avec un clic sur la carte.",
"Non avete trovato il vostro comune? Nel caso in cui voleste inserire comunque una parola, cliccate qui per localizzare in seguito il vostro comune con un click sulla carta.",
"No matching locations found. Do you want to search your location by clicking on the map?"
];

var search_map_location = [
  "Gemeinde auf der Karte suchen",
  "Chercher la commune sur la carte",
  "Cercare il comune sulla carta",
...
var nothing_found = [
  ...
  "Nothing found. Please click on an other location on the map."

];

var suggest_dialect_texts = [
"Dialekt hinzufügen",
"Ajouter un dialecte",
"Aggiungere un dialetto",
"Dodaj narečje"
];

var selected_dialect_texts = [
"Ihr neuer Dialekt",
"Votre nouveau dialecte",
"Il vostro nouvo dialetto",
"Tvoje nove narečje"
];


var submit_dialect_texts = [
"Eintragen",
"Insérer",
"Inserire",
"Vpiši"
...
var dialect_not_selected_texts=[
"Bitte wählen Sie einen Dialekt aus!",
"Choisissez un dialecte, s`il vous plaît!",
"Scelga un dialetto, per favore!",
"Prosimo izberi narečje!"
]

var alpine_agriculture = [
"Almwirtschaft",
...
var too_few_elements = [
  "Für diese Gemeinde gibt es noch sehr wenig Antworten, helfen Sie uns und schicken Sie uns Ihre Vorschläge!",
  "Il y a seulement peu de contributions pour cette commune. Aidez-nous et envoyez-nous vos propositions!",
  "Ci sono solo poche contribuzioni per questo comune. Aiutateci e mandateci le vostre proposte!",
  "Za to občino je na voljo zelo malo odgovorov, pomagajte nam in nam pošljite svoje predloge!"
];
...
function initTool(){
  ...
  if(!dialect_modal_initialized) {
          get_dialects();
          //console.log("INIT DIALECT DATATABLE");
  }
...
function bindShowListeners_Modal(){
 ...
 /*jQuery('#dialect_modal').on('shown.bs.modal',function(){
      if(dialect_modal_initialized){
        datatable_dialects.scroller.measure();
       setTimeout(function() {
        datatable_dialects.scroller.measure();
     ...
     fnInitComplete: function(settings) {
             ...
             suggest_button.on('click',function(){
                         ...
                         suggest_button_submit.on('click',function(){
                          ...
                          setTimeout(function(){
                            ...
                            /*not sure*/
                            jQuery("#regect_btn").on('click',function(){jQuery('.input_modal_content').empty();jQuery('#input_modal').modal('hide');})
                            jQuery('#input_modal').modal();
          ...
          /*end handle suggest button click*/
            ...
            jQuery('#dialect_modal .modal-body').css('padding-top','38px');
              ...
              setTimeout(function() {modal.modal('hide'); jQuery("custom_modal_backdrop").hide();}, 220); //delay to show select effect
            }
...
var arr = [];
  for (var key in array_data) {
            if (array_data.hasOwnProperty(key)) {
              ...
              if(concepts_index_by_id[va_phase_cur].hasOwnProperty(answer_concept_id)){
...
password_text = ['Passwort','Mot de passe','Password','Password']
new_acc_text = ['Neuer Account','Nouveau compte','Nuovo account','New Accout']
new_acc_text_detail = ['Account anlegen oder Daten abschicken','Créer un compte ou envoyer les données','Creare un account o inviare i dati','Account anlegen oder Daten schicken']
get_new_password = ['Neues Passwort bekommen', 'Recevoir un nouveau mot de passe', 'Ricevere una nuova password', 'Neues Passwort bekommen']
login_btn = ['Login','Login','Login','Login'];
reset_btn_text = ['Zurücksetzen','Réinitialiser','Rimettere','Reset'];
